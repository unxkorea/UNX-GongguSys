-- [요청] Railway 전환 1단계 — Railway Postgres용 전체 스키마 DDL (scripts/schema.sql의 Supabase 전용 부분 제거본)
-- 사용법: node scripts/applySchemaPg.js  (DATABASE_URL 사용, 멱등 — 여러 번 실행해도 안전)
--   Railway 대시보드 Postgres → Data 탭의 Query 창에 통째로 붙여넣어 실행해도 된다.
-- Supabase 원본(schema.sql)과의 차이:
--   - RLS(alter table ... enable row level security) 제거 — anon 접근 경로가 없음
--   - grant execute ... to anon 제거 — anon 역할 없음
--   - get_catalog_by_code의 security definer 제거 — 서버가 service 계정으로 직접 호출
...
-- 주간 카운터 원자적 증가 RPC.
--   select increment_weekly_count(1, '2026-W17')  (accountsRepo.incrementSendCount)
-- 호출하면 새 count 값을 반환.
...
--   select adjust_weekly_count(1, '2026-W17', -1)  (accountsRepo.adjustSendCount)
-- count는 0 미만으로 내려가지 않도록 greatest(..., 0)으로 클램프.
-- 운영 DB에는 DB에서 이 함수 1회 실행 필요.
...
------------------------------------------------------------
-- 5. product_photos : 제품 사진 URL (4단계 파일 저장소 전까지 Supabase 공개 URL)
------------------------------------------------------------
...
--   아래 ALTER를 DB에서 한 번 실행해야 'sending' 상태가 허용됨.
...
--    DB에서 이 블록 1회 실행 필요 — 실행 전에는 리드 저장이 check 위반으로 실패)
...
-- (Supabase 전용 grant/RLS 블록은 Railway Postgres에서 제거됨)


-- [요청] 제품 목록 필드 확장 — 기존 테이블용 마이그레이션
--   이미 배포된 DB는 위의 create table if not exists가 no-op이므로,
--   아래 ALTER를 DB에서 한 번 실행해야 새 컬럼이 추가됨.
...
--   ⚠️ 이미 배포된 운영 DB는 이 블록(12번)을 DB에서 1회 실행해야 함.
...
--   ⚠️ 이미 배포된 운영 DB는 이 블록(13번)을 DB에서 1회 실행해야 함.
------------------------------------------------------------
