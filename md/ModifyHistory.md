...
- **후속(같은 날)**: 커밋·`git push origin main` 완료, `npm run db:schema`를 Railway Postgres에 직접 실행해 반영 확인(`get_catalog_by_code` 함수에 더 이상 `memo` 없음을 실제 조회로 검증). Gonggu-Recommend(사용자가 `UNX-Gonggu`로 개명) 서비스의 첫 배포가 **Dockerfile 빌드 시도로 실패**(Root Directory를 바꿔도 서비스에 남아있던 빌드 설정이 메인 앱의 루트 [Dockerfile](../Dockerfile)을 계속 찾음). `railway.json`(Config as Code)로 Builder를 NIXPACKS로 고정하려 했으나 **Railway가 2026-08-28부터 신규 서비스의 Config as Code opt-in을 막아** 무효 — 대신 `apps/public/recommend/Dockerfile`(가벼운 자체 Dockerfile, Playwright 없음)을 직접 둬서 Root Directory 스코프 안에서 자동 감지되게 함. 이후 배포 성공, 런타임 로그로 `MAIN_APP_URL` 환경변수에 `https://` 스킴이 빠져 `fetch`가 URL 파싱에 실패하던 것 확인 → [apps/public/recommend/server.js](../apps/public/recommend/server.js)에 스킴 누락 시 자동으로 `https://`를 붙이는 정규화 로직 추가, 사용자에게는 즉시 환경변수 값 수정 안내. **환경변수 수정 후 실제 프로덕션(Gonggu-Recommend 도메인)에서 카탈로그 정상 열람 확인 — 사용자 확인 완료.**
